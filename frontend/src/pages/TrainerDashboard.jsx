...
import { LogOut, Calendar, ClipboardCheck, Users, FileText, ChevronDown, ChevronRight, MessageSquare, X } from "lucide-react";
            ...
            <div className="bg-white rounded-lg max-w-6xl w-full max-h-[90vh] overflow-auto">
              <div className="sticky top-0 bg-white border-b p-6 flex justify-between items-center">
                <div>
                  ...
                  <p className="text-gray-600">{selectedResultsSession.name}</p>
                </div>
                <Button
                  onClick={() => setSelectedResultsSession(null)}
                  variant="outline"
                  size="sm"
                >
                  <X className="w-4 h-4" />
              ...
              <div className="p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
                  <Card>
                    ...
                    <CardContent>
                      <p className="text-3xl font-bold text-blue-600">{sessionStats.totalParticipants || 0}</p>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <p className="text-3xl font-bold text-green-600">{sessionStats.attendanceRate || 0}%</p>
                    </CardContent>
                  </Card>

                  <Card>
                    <CardHeader className="pb-2">
                      <CardTitle className="text-sm text-gray-600">Pre-Tests Completed</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <p className="text-3xl font-bold text-purple-600">{sessionStats.preTestCompleted || 0}</p>
                    </CardContent>
                  </Card>

                  <Card>
                    <CardHeader className="pb-2">
                      <CardTitle className="text-sm text-gray-600">Post-Tests Completed</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <p className="text-3xl font-bold text-indigo-600">{sessionStats.postTestCompleted || 0}</p>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <p className="text-3xl font-bold text-orange-600">{sessionStats.feedbackReceived || 0}</p>
                    </CardContent>
                  </Card>
                </div>
                ...
                <div className="space-y-8">
                            ...
                            <tbody>
                              {attendance.map((record) => (
                                <tr key={record.id} className="border-b">
                                  <td className="p-2">{record.participant_name}</td>
                                  <td className="p-2">{new Date(record.check_in_time).toLocaleString()}</td>
                                  <td className="p-2">
                            ...
                            <tbody>
                              {testResults.map((result) => (
                                <tr key={result.id} className="border-b">
                                  <td className="p-2">{result.participant_name}</td>
                                  <td className="p-2 capitalize">{result.test_type}</td>
                                  <td className="p-2">{result.score}/{result.total_questions}</td>
                                  <td className="p-2">{Math.round(result.percentage)}%</td>
                                  <td className="p-2">
                  ...
                  <Card>
                    <CardHeader>
                      <CardTitle>Course Feedback Summary</CardTitle>
                    </CardHeader>
                    <CardContent>
                      {courseFeedback.length === 0 ? (
                        ...
                        <div className="space-y-4">
                          <p className="text-sm text-gray-600">
                            Total responses: {courseFeedback.length}
                          </p>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {courseFeedback.slice(0, 6).map((feedback, index) => (
                              <div key={index} className="p-4 bg-gray-50 rounded-lg">
                                <p className="text-sm font-medium text-gray-900">{feedback.participant_name}</p>
                          ...
                          {courseFeedback.length > 6 && (
                            <p className="text-sm text-gray-500 text-center">
                              And {courseFeedback.length - 6} more responses...
                            </p>
                          )}
                        </div>
                      )}
